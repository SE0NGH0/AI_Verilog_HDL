...
module uart_tx #( 
    ...
    output  reg     tx,
    output  reg     tx_done,
    output  reg     tx_busy     // data 전송중인지 여부 확인
);
    ...
    always @(posedge clk or posedge reset) begin    // tick 감지 할때마다 byte
        if (reset) begin
            r_state     <= IDLE;    // state transition
            r_bit_cnt   <= 0;       // bit count to transmission
            r_data_reg  <= 0;       // 전송할 1byte
            tx_done     <= 0;       
            tx_busy     <= 0;       
            tx          <= 1;       
        end else begin
